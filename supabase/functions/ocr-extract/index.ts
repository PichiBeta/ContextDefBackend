...
const AZURE_ANALYZE_URL = `${AZURE_ENDPOINT}/documentintelligence/documentModels/prebuilt-read:analyze?api-version=2024-02-29-preview`;
const MAX_FILES = 10;
...
async function pollAzure(operationUrl) {
  while(true){
...
function normalizeParagraphs(paragraphs) {
  return paragraphs.map((p)=>p.content.replace(/\s+/g, " ").trim()).filter((p)=>p.length > 0).join("\n\n");
}
...
async function ocrSingleFile(f) {
  const isPDF = f.type === "application/pdf";
  const analyzeUrl = isPDF ? `${AZURE_ANALYZE_URL}&pages=1-10` : AZURE_ANALYZE_URL;
...
Deno.serve(async (req)=>{
  ...
  let texts;
  try {
    texts = await mapWithConcurrency(files, OCR_CONCURRENCY, async (f)=>{
