...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { title, genre, language_code, visibility, content } = payload;
    if (typeof title !== "string" || title.trim().length === 0) {
      return jsonResponse({
        ok: false,
        error: 'Invalid payload: non-empty "title" required'
      }, 400);
    }
    if (typeof genre !== "string" || genre.trim().length === 0) {
      return jsonResponse({
        ok: false,
        error: 'Invalid payload: non-empty "genre" required'
      }, 400);
    }
    if (typeof language_code !== "string" || language_code.trim().length === 0) {
      return jsonResponse({
        ok: false,
        error: 'Invalid payload: non-empty "language_code" required'
      }, 400);
    }
    ...
    if (typeof content !== "string" || content.trim().length === 0) {
      return jsonResponse({
        ok: false,
        error: 'Invalid payload: non-empty "content" required'
      }, 400);
    }
    const v = visibility;
    ...
    const { data: reading, error: insertErr } = await supabase.from("readings").insert({
      owner_id,
      title: title.trim(),
      genre: genre.trim(),
    ...
    const { error: saveErr } = await supabase.from("user_saved_readings").insert({
      user_id: owner_id,
      reading_id
    });
    if (saveErr) {
    ...
    const { error: uploadErr } = await supabase.storage.from("readings").upload(objectName, bytes, {
    ...
    if (uploadErr) {
      ...
      const { error: markErr } = await supabase.from("readings").update({
        status: "failed",
    ...
    const { error: updErr } = await supabase.from("readings").update({
      status: "uploaded",
