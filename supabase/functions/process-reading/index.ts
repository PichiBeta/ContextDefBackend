...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (typeof body.content_updated_at !== "string" || body.content_updated_at.trim().length === 0) {
    ...
    const { data: file, error: dlErr } = await supabase.storage.from("readings").download(storage_path);
    if (dlErr || !file) {
    ...
    const { error: upErr } = await supabase.storage.from("readings").upload(structurePath, new Blob([
    ...
    const { data: updated, error: updErr } = await supabase.from("readings").update({
      difficulty: difficultyInt,
      status: "processed",
      error_message: null
    }).eq("id", reading_id).eq("content_updated_at", content_updated_at).select("id").maybeSingle();
