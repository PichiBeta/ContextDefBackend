...
Deno.serve(async (req) => {
  ...
  if (!reading.embedding){
    return new Response(JSON.stringify({error: "No embedding associated with reading"}), {status: 422})
  }
  ...
  if (user.num_vectors === 0) {
    ...
    if (avgError) {
      return new Response(JSON.stringify({ error: avgError.message }), { status: 500 });
    }
  ...
  return new Response(JSON.stringify({ success: true, num_vectors: newNumVectors }), {
